...
**Objetivo finalizado sem itens.** Se a cascata da RN21 esvaziar um objetivo **finalizado**, ele continua **Finalizado**, conforme a precedência da tabela acima: excluir um item do catálogo não desfaz uma conquista. Ele recebe `semItens`, mas o aviso "Sem itens" **não é exibido** em objetivos finalizados. Se o usuário adicionar a ele uma unidade não obtida, vale a RN15: `finalizado` volta a `false` e `finalizadoEm` a `null` na mesma escrita, e o objetivo passa a **Aguardando**, já que nenhuma das unidades presentes foi obtida.
